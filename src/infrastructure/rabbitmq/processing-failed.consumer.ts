...
@Injectable()
export class ProcessingFailedConsumer implements OnModuleInit {
  constructor(
  ...
  async onModuleInit(): Promise<void> {
    ...
    await channel.consume('processing.failed', (message) => {
      if (!message) {
        return;
      }
  ...
  private parse(content: unknown): ProcessingFailedDto {
    ...
    if (
      ...
      throw new ProcessingRequestDomainError(
        'Invalid ProcessingFailed payload',
      );
    }
